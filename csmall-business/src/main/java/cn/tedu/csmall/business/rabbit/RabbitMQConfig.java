...
public class RabbitMQConfig {
    ...
    public static final String BUS_EX="bus_ex";
    public static final String BUS_ROUT="bus_rout";
    public static final String BUS_QUEUE="bus_queue";

    // 绑定关系中,交换机和队列是实际对象,直接实例化保存到Spring容器
    @Bean
    public DirectExchange stockDirectExchange(){
        return new DirectExchange(BUS_EX);
    }
    @Bean
    public Queue stockQueue(){
        return new Queue(BUS_QUEUE);
    }
    // 路由Key是关系对象,保存方式特殊
    @Bean
    public Binding stockBinding(){
        return BindingBuilder.bind(stockQueue())
                            .to(stockDirectExchange()).with(BUS_ROUT);
    }
